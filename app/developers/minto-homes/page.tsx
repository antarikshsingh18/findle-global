...
export const metadata: Metadata = {
  title: 'Minto Homes | Pre-Construction & New Communities in Ontario | Findle',
  description: 'Explore Minto Homes\' pre-construction condominiums, townhomes, detached houses, and master-planned communities across Ottawa, Oakville, Markham, and the GTA on Findle.',
};

export default async function MintoHomesPage() {
  ...
  // 2. Filter properties specifically for Minto Homes using clean normalization
  const cleanString = (str: string) => 
    str.toLowerCase().replace(/-(homes|communities|group|developments|inc|ltd|corporation)|(homes|communities|group|developments|inc|ltd|corporation)/g, '').replace(/[^a-z0-9]/g, '');

  const targetSlug = cleanString('minto homes');

  const developerProperties = allListings.filter((item) => {
    if (!item.developer) return false;
    const itemDev = cleanString(item.developer);
    return itemDev.includes('minto') || targetSlug.includes(itemDev);
  });
          ...
          <div className="grid items-center gap-6 lg:grid-cols-[1.2fr_0.8fr]">
            <div>
              <div className="mb-3 text-[9px] uppercase tracking-[0.22em] text-indigo-400">
                Verified Developer Profile // Established 1955 in Ottawa, Ontario
              </div>
              ...
              <p className="mt-4 max-w-2xl text-sm leading-6 text-slate-300 font-sans">
                Minto Homes is one of Canada's most established homebuilders, with a history spanning more than seven decades of residential development. As part of the Minto Group, the company has played a significant role in shaping communities across Ontario through the development of detached homes, townhomes, condominiums, and large master-planned neighbourhoods.
              </p>
            </div>
            ...
            <div className="relative">
              <div className="overflow-hidden rounded-xl border border-slate-700 bg-slate-950 flex items-center justify-center p-6">
                ...
                <img
                  src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS3XUNLTi7fMqltRAV97hE0P4QZM8uRfRk4S6kRgWau0w&s=10"
                  alt="Minto Homes logo"
              ...
              <div className="mt-3 grid grid-cols-2 gap-3">
                <div className="rounded-lg border border-slate-800 bg-slate-950/80 p-3">
                  ...
                  <span className="mt-1 block text-xl font-bold text-emerald-400">{totalProjects > 0 ? totalProjects : 6}</span>
                </div>
                <div className="rounded-lg border border-slate-800 bg-slate-950/80 p-3">
        ...
        <section className="mb-16 grid grid-cols-1 md:grid-cols-2 gap-8 border border-slate-800 bg-slate-900/20 p-6 sm:p-8 rounded-2xl">
          <div>
            ...
            <ul className="space-y-3 text-sm text-slate-300 font-sans">
              <li><strong className="text-slate-100 font-mono text-xs uppercase">Developer:</strong> Minto Homes</li>
              <li><strong className="text-slate-100 font-mono text-xs uppercase">Parent Company:</strong> Minto Group</li>
              <li><strong className="text-slate-100 font-mono text-xs uppercase">Founded:</strong> 1955</li>
              <li><strong className="text-slate-100 font-mono text-xs uppercase">Headquarters:</strong> Ottawa, Ontario</li>
              <li><strong className="text-slate-100 font-mono text-xs uppercase">Property Types:</strong> Condos, Townhomes, Detached Homes</li>
            </ul>
          </div>
          ...
          <div>
            ...
            <div className="space-y-4 text-sm text-slate-300 font-sans">
              <div>
                ...
                <p className="text-xs mt-1">Key urban and suburban residential nodes including Toronto, Oakville (Oakvillage, North Oak), Markham (Union Village), and Mississauga.</p>
              </div>
              <div>
                <strong className="text-white font-mono text-xs uppercase block text-indigo-400">Ottawa Region</strong>
                <p className="text-xs mt-1">Foundational home market featuring extensive master-planned communities across Ottawa, Kanata (Brookline, Arcadia), Stittsville (Abbott's Run), and Manotick (Mahogany).</p>
              </div>
            </div>
          </div>
        </section>
          ...
          {developerProperties.length > 0 ? (
            ...
            <div className="grid grid-cols-1 gap-y-12 gap-x-8 sm:grid-cols-2 lg:grid-cols-3">
              ...
              <div className="border border-slate-800 bg-slate-900/40 rounded-xl p-6">
                ...
                <h3 className="text-lg font-bold text-white mt-2">Oakvillage</h3>
                <p className="text-xs text-slate-400 mt-1">Oakville, Ontario</p>
                <div className="mt-4 pt-4 border-t border-slate-800 text-xs text-slate-300">Condominiums & Townhomes</div>
              </div>
              <div className="border border-slate-800 bg-slate-900/40 rounded-xl p-6">
                <span className="text-[9px] text-emerald-400 font-bold uppercase">● Active Community</span>
                <h3 className="text-lg font-bold text-white mt-2">Union Village</h3>
                <p className="text-xs text-slate-400 mt-1">Markham, Ontario</p>
                <div className="mt-4 pt-4 border-t border-slate-800 text-xs text-slate-300">Townhomes & Detached Homes</div>
              </div>
              <div className="border border-slate-800 bg-slate-900/40 rounded-xl p-6">
                <span className="text-[9px] text-emerald-400 font-bold uppercase">● Active Community</span>
                <h3 className="text-lg font-bold text-white mt-2">Mahogany</h3>
                <p className="text-xs text-slate-400 mt-1">Manotick (Ottawa), Ontario</p>
                <div className="mt-4 pt-4 border-t border-slate-800 text-xs text-slate-300">Detached Homes & Townhomes</div>
              </div>
              <div className="border border-slate-800 bg-slate-900/40 rounded-xl p-6">
                <span className="text-[9px] text-emerald-400 font-bold uppercase">● Active Community</span>
                <h3 className="text-lg font-bold text-white mt-2">Brookline</h3>
                <p className="text-xs text-slate-400 mt-1">Kanata, Ontario</p>
                <div className="mt-4 pt-4 border-t border-slate-800 text-xs text-slate-300">Detached Homes & Townhomes</div>
              </div>
              <div className="border border-slate-800 bg-slate-900/40 rounded-xl p-6">
                <span className="text-[9px] text-emerald-400 font-bold uppercase">● Active Community</span>
                <h3 className="text-lg font-bold text-white mt-2">Arcadia</h3>
                <p className="text-xs text-slate-400 mt-1">Kanata, Ontario</p>
                <div className="mt-4 pt-4 border-t border-slate-800 text-xs text-slate-300">Single Family Homes & Townhomes</div>
              </div>
              <div className="border border-slate-800 bg-slate-900/40 rounded-xl p-6">
                <span className="text-[9px] text-emerald-400 font-bold uppercase">● Active Community</span>
                <h3 className="text-lg font-bold text-white mt-2">Abbott's Run</h3>
                <p className="text-xs text-slate-400 mt-1">Stittsville, Ontario</p>
                <div className="mt-4 pt-4 border-t border-slate-800 text-xs text-slate-300">Detached Homes & Townhomes</div>
              </div>
            </div>
          )}
        ...
        <section className="mt-20 border-t border-slate-800 pt-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm font-sans">
            <div className="border border-slate-800 bg-slate-900/30 p-6 rounded-xl">
              <h3 className="font-bold text-white font-mono uppercase text-xs mb-2 text-indigo-400">Who is Minto Homes?</h3>
              <p className="text-slate-300">Minto Homes is a Canadian homebuilder founded in 1955 and operating as part of the Minto Group. The company develops condominiums, townhomes, detached homes, and master-planned communities across Ontario.</p>
            </div>
            <div className="border border-slate-800 bg-slate-900/30 p-6 rounded-xl">
              <h3 className="font-bold text-white font-mono uppercase text-xs mb-2 text-indigo-400">Where does Minto Homes build?</h3>
              <p className="text-slate-300">Minto Homes builds in major growth markets across Ontario including Ottawa, Oakville, Markham, Toronto, Kanata, Stittsville, and Manotick.</p>
            </div>
            <div className="border border-slate-800 bg-slate-900/30 p-6 rounded-xl">
              <h3 className="font-bold text-white font-mono uppercase text-xs mb-2 text-indigo-400">What types of homes does Minto Homes build?</h3>
              <p className="text-slate-300">The company develops condominiums, flexible townhomes, spacious detached single-family homes, and comprehensive master-planned communities.</p>
            </div>
            <div className="border border-slate-800 bg-slate-900/30 p-6 rounded-xl">
              <h3 className="font-bold text-white font-mono uppercase text-xs mb-2 text-indigo-400">What are Minto Homes' current communities?</h3>
              <p className="text-slate-300">Current active communities include Oakvillage, Union Village, Mahogany, Brookline, Arcadia, and Abbott's Run, alongside future planned expansions like North Oak.</p>
            </div>
          </div>
        </section>
